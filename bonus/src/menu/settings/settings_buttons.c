...
static void	render_button(t_data *data, t_button_info *button, t_vec2i pos_mouse)
{
	...
	{
		mlx_put_image_to_window(data->graphics.mlx, data->graphics.win,
			data->graphics.settings.back_button.img.img, button->pos.x,
			button->pos.y);
	}
...
static t_button_info	*get_settings_buttons(t_data *data)
{
	...
	buttons[0] = (t_button_info){{700, 250}, sizes, MOVE_FORWARD, MOVE_FORWARD,
		"Move forward", data->settings.key_move_forward};
	buttons[1] = (t_button_info){{700, 350}, sizes, MOVE_BACKWARD,
		MOVE_BACKWARD, "Move backward", data->settings.key_move_backward};
	buttons[2] = (t_button_info){{700, 450}, sizes, MOVE_LEFT, MOVE_LEFT,
		"Move left", data->settings.key_move_left};
	buttons[3] = (t_button_info){{700, 550}, sizes, MOVE_RIGHT, MOVE_RIGHT,
		"Move right", data->settings.key_move_right};
	buttons[4] = (t_button_info){{700, 650}, sizes, SPRINT, SPRINT, "Sprint",
		data->settings.key_sprint};
	buttons[5] = (t_button_info){{700, 750}, sizes, MAP_ZOOM, MAP_ZOOM,
		"Map zoom", data->settings.key_map_zoom};
	buttons[6] = (t_button_info){{1250, 250}, sizes, SHOW_FLOOR_AND_CEILING,
		SHOW_FLOOR_AND_CEILING, "Floor and ceiling",
		data->settings.key_show_floor_and_ceiling};
	buttons[7] = (t_button_info){{1250, 350}, sizes, ROTATE_LEFT, ROTATE_LEFT,
		"Rotate left", data->settings.key_rotate_left};
	buttons[8] = (t_button_info){{1250, 450}, sizes, ROTATE_RIGHT, ROTATE_RIGHT,
		"Rotate right", data->settings.key_rotate_right};
	return (buttons);
}
